...
class UploadSchema(Schema):
    file = fields.Raw(required=True, error_messages={
                      "required": "File is required."}, validate=UploadValidator().validate)
    ...
    def post(self):
        try:
            ...
            if parsed_file is not None:
                ...
                file = CAFFFiles(
                    date=date.today(), caff_location=path_to_file, img_location=path_to_file)
                db.session.add(file)
                db.session.commit()

                # return str(path_to_file)  # TODO: --- real
                # TODO: most mindent listáz, bár nem feltétlen baj
                files = JsonHelper.search_to_json(CAFFFiles.query.all())
